...
static NSString* const HUD_BUNDLE = @"LibDotCR.bundle/HUD.bundle/";

#define HUD_BUNDLE_RESOURCE(name) [HUD_BUNDLE stringByAppendingString:name]
...
+ (void)showImage:(UIImage *)image status:(NSString *)string {
    NSTimeInterval displayInterval = 1.5f;//[[SVProgressHUD sharedView] displayDurationForString:string];
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED >= 70000
    return [UIImage imageNamed:HUD_BUNDLE_RESOURCE(@"success-black")];
#else
    return [UIImage imageNamed:HUD_BUNDLE_RESOURCE(@"success.png")];
#endif
}
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED >= 70000
    return [UIImage imageNamed:HUD_BUNDLE_RESOURCE(@"error-black")];
#else
    return [UIImage imageNamed:HUD_BUNDLE_RESOURCE(@"error.png")];
#endif
}
