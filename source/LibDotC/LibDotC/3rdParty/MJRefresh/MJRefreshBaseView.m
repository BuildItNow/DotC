...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        _mjDefaultContent = STRONG_OBJECT(UIView, init);
        _mjDefaultContent.backgroundColor = [UIColor clearColor];
        
        [self addSubview:_mjDefaultContent];
        
        // 2.时间标签
        _lastUpdateTimeLabel = [self labelWithFontSize:12];
        [_lastUpdateTimeLabel retain];
        [_mjDefaultContent addSubview:_lastUpdateTimeLabel];
        
        // 3.状态标签
        _statusLabel = [self labelWithFontSize:13];
        [_statusLabel retain];
        [_mjDefaultContent addSubview:_statusLabel];
...
- (void)setFrame:(CGRect)frame
{
    ...
    _mjDefaultContent.frame = CGRectMake(0, 0, self.frame.size.width, self.frame.size.height);
...
- (void)removeFromSuperview
{
    ...
    _scrollView = nil;
    self.delegate   = nil;
    self.beginRefreshingBlock = nil;
    self.refreshStateChangeBlock = nil;
    self.endStateChangeBlock = nil;
